...
local function moveBetweenActiveInactive(username, from, to)
    ...
    if AshenMPRanking.sandboxSettings.perkScores then
        ...
        from.perkScores.farmingcategory[username] = nil
...
local function initServer()
    ...
    for username,v in pairs(lastUpdate) do
        if AshenMPRanking.sandboxSettings.perkScores then
            ...
            if ladder.perkScores.physicalcategory[username] ~= nil then
                if ladder.perkScores.physicalcategory[username] > AshenMPRanking.sandboxSettings.physicalcategoryMaxScore then
...
local function onPlayerData(player, playerData)
    ...
    if AshenMPRanking.sandboxSettings.debugMode then
        -- print all playerData on a single line
        local debugLine = "AMPR DEBUG - PlayerData for " .. playerData.username .. ": "
        for k,v in pairs(playerData) do
            debugLine = debugLine .. k .. "=" .. tostring(v) .. " "
        end
        print(debugLine)

        -- print ladder before update for this player on one line
        local ladderLine = "AMPR DEBUG - Ladder before update for " .. playerData.username .. ": "
        for k,v in pairs(ladder) do
